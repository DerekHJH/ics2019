...
make_EHelper(sbb) {
 ...
 /* if (id_dest->width != 4) {
  ...
  // update CF
  rtl_is_sub_carry(&s3,&s0,&id_dest->val,&id_src->val,id_dest->width);
  rtl_is_sub_carry(&s4,&s1,&s0,&s2,id_dest->width);
  rtl_or(&s3, &s3, &s4);
  rtl_set_CF(&s3);

  // update OF
  rtl_is_sub_overflow(&s3, &s0, &id_dest->val, &id_src->val, id_dest->width);
  rtl_is_sub_overflow(&s4, &s1, &s0, &s2, id_dest->width);
  rtl_or(&s3, &s3, &s4);
	rtl_set_OF(&s3);
